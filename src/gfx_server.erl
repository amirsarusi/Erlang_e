...
handle_event(#wx{obj = SendMsgBtn, event = #wxCommand{type = command_button_clicked}},
  ...
  EtsTab = ets:tab2list(?locationEts),
  io:format("table: ~p~n",[EtsTab]),
...
handle_cast({messageSent,Pathlist}, State = #state{}) ->
...
%% Local functions
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
makeMulticast([],_Src,_Msg) -> [];
makeMulticast(OutputList,Src,Msg) ->
  [{Src,hd(OutputList),Msg} | makeMulticast(Src,tl(OutputList),Msg)].
...
init_layout(Mode,Node) ->
  Wx = wx:new(),
  Frame = wxFrame:new(Wx,-1,"RPL Simulation",[{size,{?MapSize + 100,?MapSize + 100}}]),
...
draw(State = #state{panel = Panel,node_list_q_1 = Q1,node_list_q_2 = Q2,
  node_list_q_3 = Q3,node_list_q_4 = Q4,src = Src,srcTextBox = SrcTextBox, msgState = MsgState}) ->
...
update(Panel,[{nodePid,Pid}|[]],Q1,Q2,Q3,Q4) ->
  ...
  drawNode({X,Y},NodeType,MsgRole,Panel),
